...
class ClientConfigurations(models.Model):
    ...
    def _cron_generate_notifications(self):
        ...
        """
        # Define Rules order based con configuration
        order = True if self.priority == 'desc' else False
        rule_lists = {}
        records = self.sudo().env['lgps.client_configuration'].search([('active', '=', True)])
        _logger.warning('Registros : %s', records)

        # Iterate records in this configuration
        for record in records:
            _logger.warning('Record: %s', record.name)
            _logger.warning('Rules found: %s', record.rule_ids)
            ...
            for infraction in rule_lists:
                if len(rule_lists[infraction]) > 0:
                    ...
                    r = self.create_notification(record, infraction, rule_lists[infraction])
                    _logger.warning('Notification created: %s', r.name)
    ...
    def test_generate_notifications(self):
        ...
        # Iterate records in this configuration
        for record in self.sudo().env['lgps.client_configuration'].search([('active', '=', True)]):
            _logger.warning('Record: %s', record.name)
            _logger.warning('Rules found: %s', record.rule_ids)
            ...
            for infraction in rule_lists:
                if len(rule_lists[infraction]) > 0:
                    _logger.warning('Infraction: %s', rule_lists[infraction])
                    record = self.create_notification(record, infraction, rule_lists[infraction])
    ...
    def create_notification(self, record, rule, devices):
        ...
        dictionary = {
            'description': 'Notificatión generada por sistema',
            'rule_id': rule,
            'client_id': record.client_id.id,
