...
class ClientConfigurations(models.Model):
    ...
    def _cron_generate_notifications(self):
        ...
        for record in self.sudo().env['lgps.client_configuration'].search([('operative', '=', True)]):
            ...
            if record.rule_ids:
                ...
                #_logger.error('Devices foune: %s', gps_devices)
                    ...
                    if gps_devices:
                        for device in gps_devices:
                            ...
                            if device.last_rule_applied:
                                ...
                                for rule in sorted_rules:
                                    #_logger.warning('Last Rule: %s vs Current Rule %s',
                                                    #device.last_rule_applied.hours_rule, rule.hours_rule)
                                    if rule.hours_rule > device.last_rule_applied.hours_rule:
                                        #_logger.warning('device.last_report: %s vs rule.hours_rule %s',
                                                        #device.last_report, rule.hours_rule)
